...
func pageTemplateTemplateManager(w http.ResponseWriter, r *http.Request) {
	...
	modalBody.AddChild(hb.NewDiv().Attr("class", "form-group").AddChild(hb.NewLabel().HTML("Title")).AddChild(hb.NewInput().Attr("class", "form-control").Attr("v-model", "templateCreateModel.title")))
...
const TemplateManager = {
	...
	methods: {
		...
		templateCreate(){
			var title = this.templateCreateModel.title;
		    $.post(templateCreateUrl, {title: title}).done((result)=>{
